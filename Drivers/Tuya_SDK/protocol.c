...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_TEMP_CURRENT, 0.1*Temperature); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_VALUE, 0.01*Humidity); //VALUE型数据上报;
...
//    mcu_dp_string_update(DPID_UART,当前透传指针,当前透传数据长度); //STRING型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_uart_handle
功能描述 : 针对DPID_UART的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_uart_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_UART,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_UART:
            //透传处理函数
            ret = dp_download_uart_handle(value,length);
        break;
